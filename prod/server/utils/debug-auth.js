...
async function debugAuth() {
  ...
  try {
    ...
    const [users] = await connection.execute('SELECT id, email, password_hash, first_name, last_name, role, is_active FROM orthodoxmetrics_auth_db.users WHERE email = ?', ['admin@orthodoxmetrics_db.com']);
    ...
    if (users.length > 0) {
      ...
      if (!isMatch) {
        ...
        await connection.execute('UPDATE orthodoxmetrics_auth_db.users SET password_hash = ? WHERE id = ?', [newHash, user.id]);
