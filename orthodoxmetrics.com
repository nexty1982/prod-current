server {
    ...
    # ---------- No-cache for index.html (CRITICAL: prevents old SPA from being cached) ----------
    location = /index.html {
        expires off;
    ...
    # ---------- No-cache for all other HTML files ----------
    location ~* \.(html)$ {
        expires off;
        add_header Cache-Control "no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0" always;
        add_header Pragma "no-cache" always;
    ...
    # ---------- SPA entry (routes all requests to index.html, but index.html itself is not cached) ----------
    # Try the file/dir; if missing, fall back to SPA index.html (so app routes don't 404)
    location / {
        try_files $uri $uri/ /index.html;
        # No expires here - index.html caching is handled by the location = /index.html block above
    }

    # ---------- Static assets (JS/CSS with cache-busting) ----------
    # Main JS bundles should have cache-busting via Vite's hash filenames
    location ~* \.(js|css)$ {
        # Cache for 1 year if filename has hash (Vite's cache-busting)
        # Otherwise no-cache for development builds
        expires 1y;
