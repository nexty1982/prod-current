// Support both development (../src/api/logs) and production (../api/logs) paths
const { safeRequire } = require('../utils/safeRequire');

// Safe require for logs module - tries multiple paths and falls back to console.log
const logsModule = safeRequire(
  '../api/logs',
  () => safeRequire(
    '../src/api/logs',
    () => ({
      logMessage: function(level, service, message, details, req) {
        // Fallback to console.log if logs module not available
...
const logMessage = logsModule.logMessage || function(level, service, message, details, req) {
  console.log(`[${level.toUpperCase()}] [${service}] ${message}`, details || '');
};

// Middleware to log all API requests
const requestLogger = (req, res, next) => {
    // Skip build.meta.json and other noise endpoints
    if (req.path === '/build.meta.json' || 
    ...
    // Log the incoming request (only once)
    if (!res.locals.__requestLogged) {
        res.locals.__requestLogged = true;
    ...
    // Override res.end to log the response (only once)
    const originalEnd = res.end;
    res.end = function (chunk, encoding) {
        const duration = Date.now() - startTime;

        // Only log if not already logged by another middleware
        if (!res.locals.__responseLogged) {
