...
class NotificationService {
    ...
    async createNotification(userId, typeName, title, message, options = {}) {
        try {
            ...
            // Fetch the full notification with type info for WebSocket emission
            const [notificationRows] = await getAppPool().query(`
                SELECT 
                    n.*,
            ...
            // Emit WebSocket event to user if WebSocket service is available
            try {
                const websocketService = require('../services/websocketService');
                if (websocketService && websocketService.io && notificationRows.length > 0) {
                    ...
                    // Emit directly to user's socket room (notification already created in DB)
                    if (websocketService.userSockets && websocketService.userSockets.has(userId)) {
                ...
                // WebSocket emission is optional - don't fail notification creation
                console.warn('Could not emit WebSocket notification (non-fatal):', wsError.message);
            }
...
router.get('/admin/notifications/queue', requireRole(['super_admin']), async (req, res) => {
    try {
        ...
        // Get all notifications with their details
        // Group by title + message + target_audience + date to show notification campaigns
        const [allNotifications] = await getAppPool().query(`
            SELECT 
                n.id,
                n.title,
                n.message,
            ...
            FROM notifications n
            JOIN notification_types nt ON n.notification_type_id = nt.id
            LEFT JOIN orthodoxmetrics_db.users u ON n.user_id = u.id
            ORDER BY n.created_at DESC
            LIMIT 500
        `);
        
        // Group notifications by campaign (same title, message, target_audience, and date)
        ...
        allNotifications.forEach(notif => {
            ...
            const campaignKey = `${notif.title}|${notif.message}|${targetAudience}|${notif.created_at.toISOString().split('T')[0]}`;
            ...
            if (!campaignMap.has(campaignKey)) {
                ...
                if (data.is_draft === true) {
                    status = 'draft';
                } else if (data.scheduled_at && new Date(data.scheduled_at) > new Date()) {
                    status = 'pending';
                }
                ...
                campaignMap.set(campaignKey, {
                    ...
                    scheduled_at: data.scheduled_at || notif.created_at,
                    ...
                    is_custom: data.custom_notification === true
                });
            }
            
            const campaign = campaignMap.get(campaignKey);
            campaign.total_sent++;
            if (notif.is_read) campaign.read_count++;
            if (notif.is_dismissed) campaign.dismissed_count++;
        });
        
        // Convert map to array and sort by created_at DESC
        const processedQueue = Array.from(campaignMap.values())
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch notification queue', error: error.message });
    }
});
...
router.post('/admin/notifications/custom', requireRole(['super_admin']), async (req, res) => {
    try {
        ...
        // Handle role-based targeting (e.g., 'role:super_admin', 'role:admin', etc.)
        if (target_audience && target_audience.startsWith('role:')) {
            const roleName = target_audience.replace('role:', '');
            userQuery = 'SELECT id FROM orthodoxmetrics_db.users WHERE role = ? AND is_active = 1';
            userParams = [roleName];
        } else {
        ...
        // Get 'system_alert' notification type ID
        const [typeRows] = await getAppPool().query(
            'SELECT id FROM notification_types WHERE name = ? AND is_active = TRUE',
            ['system_alert']
        );
        ...
        if (typeRows.length === 0) {
            return res.status(500).json({ 
                success: false, 
                message: 'system_alert notification type not found. Please ensure notification types are initialized.' 
            });
        }
        
        const systemAlertTypeId = typeRows[0].id;
        
        // If sending now, create notifications for all target users
        if (shouldSendNow) {
            const notifications = users.map(user => [
                user.id,
                systemAlertTypeId, // Use 'system_alert' notification type ID
                title,
                message,
                JSON.stringify({
                    target_audience,
                    ...
                    created_by: req.session.user?.email || req.user?.email || 'system'
                }),
                priority,
                action_url,
                action_text,
            ...
            `, [
                req.session.user?.id || req.user?.id, // Store under the creator's ID for drafts
                systemAlertTypeId, // system_alert type ID
                title,
                message,
                JSON.stringify({
                    target_audience,
            ...
            console.log(`📢 Custom notification "${title}" ${status} by ${req.session.user?.email || req.user?.email || 'system'}`);
            res.json({ 
                success: true, 
