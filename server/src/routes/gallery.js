...
router.get('/churches/:id/paths', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const church = rows[0];
    const defaultUploadBase = process.env.UPLOAD_BASE_PATH || '/var/www/orthodoxmetrics/prod/uploads';
...
/**
 * POST /api/gallery/churches/:id/custom-library/upload
 * Upload an image to the church's custom_images directory
 * Destination: uploads/om_church_{id}/custom_images/
 */
router.post('/churches/:id/custom-library/upload', requireAuth, (req, res) => {
  try {
    const churchId = parseInt(req.params.id);
    if (!churchId || isNaN(churchId)) {
      return res.status(400).json({ success: false, error: 'Valid church ID is required' });
    ...
    const destDir = path.join(uploadsBase, `om_church_${churchId}`, 'custom_images');
    ...
    // Configure multer with church-specific destination
    const churchUpload = multer({
      storage: multer.diskStorage({
        destination: (r, file, cb) => cb(null, destDir),
        filename: (r, file, cb) => {
          // Sanitize and deduplicate filename
          let sanitized = file.originalname.replace(/[^a-zA-Z0-9._-]/g, '_');
          const ext = path.extname(sanitized);
    ...
    churchUpload.single('image')(req, res, (multerErr) => {
      if (multerErr) {
        console.error(`[custom-library-upload] Multer error for church ${churchId}:`, multerErr.message);
        return res.status(400).json({
          success: false,
          error: multerErr.code === 'LIMIT_FILE_SIZE' ? 'Maximum file size is 10MB' : multerErr.message,
        });
      }
      ...
      const serve_url = `/uploads/om_church_${churchId}/custom_images/${filename}`;

      console.log(`[custom-library-upload] Church ${churchId}: uploaded ${filename} (${size} bytes)`);
    ...
    console.error('[custom-library-upload] Error:', error);
    if (!res.headersSent) {
...
/**
 * GET /api/gallery/churches/:id/custom-library
 * List images from church custom_images dir + global custom_images fallback
 * Returns: { images: [{ filename, serve_url, source }] }
 */
router.get('/churches/:id/custom-library', requireAuth, async (req, res) => {
  try {
    const churchId = parseInt(req.params.id);
    if (!churchId || isNaN(churchId)) {
      return res.status(400).json({ success: false, error: 'Valid church ID is required' });
    ...
    const uploadsBase = path.resolve(__dirname, '..', '..', '..', 'uploads');
    const churchDir = path.join(uploadsBase, `om_church_${churchId}`, 'custom_images');
    const globalDir = path.join(uploadsBase, 'global', 'custom_images');

    const imageExtensions = ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg'];
    const images = [];
    const seen = new Set();

    // Scan a directory and collect image files
    const scanDir = (dir, source) => {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (seen.has(entry.name)) continue; // church version takes precedence
          seen.add(entry.name);
          let stats;
          try { stats = fs.statSync(path.join(dir, entry.name)); } catch (e) { stats = null; }
          images.push({
            filename: entry.name,
            serve_url: `/uploads/${source === 'church' ? `om_church_${churchId}` : 'global'}/custom_images/${entry.name}`,
            source,
            size: stats ? stats.size : 0,
            modified: stats ? stats.mtime : null,
          });
    ...
    // Church-specific first (takes precedence), then global fallback
    scanDir(churchDir, 'church');
    ...
    res.json({
      ...
      images,
      count: images.length,
      paths: { church: churchDir, global: globalDir },
    });
  } catch (error) {
    console.error('Error listing custom library:', error);
    res.status(500).json({ success: false, error: 'Failed to list custom image library' });
  }
});
