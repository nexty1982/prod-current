...
router.get('/', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    // Read optional tenant context
    const churchId = Number(req.query.church_id || req.headers['x-om-church-id'] || req.user?.default_church_id || 0);
    const { search, status, limit = 100, offset = 0 } = req.query;

    // Add structured logging
    console.info('[AdminUsers]', { 
      requestId,
      churchId, 
    ...
    // Build the query with fixed SQL - use only c.church_name
    let query = `
      SELECT
        u.id,
        u.email,
    ...
    // Apply tenant filter if churchId is provided
    if (churchId) {
    ...
    console.log('🔍 Executing users query:', query.replace(/\s+/g, ' ').trim());
    ...
    // Apply same tenant filter to count
    if (churchId) {
      countQuery += ` AND u.church_id = ?`;
    ...
    console.error(`[AdminUsers:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
...
router.post('/', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:Create]', { 
      requestId,
      email, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    console.log(`✅ User created: ${email} (ID: ${userId}) by ${req.user?.email || req.session?.user?.email}`);
    ...
    console.error(`[AdminUsers:Create:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
...
router.patch('/:userId/reset-password', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:ResetPassword]', { 
      requestId,
      userId, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    console.log(`🔑 Password reset for user: ${user.email} by ${req.user?.email || req.session?.user?.email}`);
    ...
    console.error(`[AdminUsers:ResetPassword:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
...
router.delete('/:id', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:Delete]', { 
      requestId,
      userId: id, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    console.log(`🗑️ User deleted: ${user.email} (ID: ${id}) by ${req.user?.email || req.session?.user?.email}`);
    ...
    console.error(`[AdminUsers:Delete:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
...
router.post('/:userId/lockout', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:Lockout]', { 
      requestId,
      userId, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    await DatabaseService.queryPlatform(lockoutQuery, [
      req.user?.email || req.session?.user?.email || 'System',
      lockoutReason,
      userId
    ]);
    ...
    await DatabaseService.queryPlatform(logQuery, [
      userId,
      JSON.stringify({
        locked_by: req.user?.email || req.session?.user?.email || 'System',
    ...
    console.log(`🔒 User locked out: ${req.user?.email || req.session?.user?.email || 'System'} locked out user ${user.email} (terminated ${terminatedSessions} sessions)`);
    ...
    console.error(`[AdminUsers:Lockout:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
...
router.post('/:userId/unlock', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:Unlock]', { 
      requestId,
      userId, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    await DatabaseService.queryPlatform(logQuery, [
      userId,
      JSON.stringify({
        unlocked_by: req.user?.email || req.session?.user?.email || 'System'
      }),
      req.ip || 'unknown',
      req.get('User-Agent') || 'unknown'
    ]);

    console.log(`🔓 User unlocked: ${req.user?.email || req.session?.user?.email || 'System'} unlocked user ${user.email}`);
    ...
    console.error(`[AdminUsers:Unlock:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
...
router.put('/:id/toggle-status', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:ToggleStatus]', { 
      requestId,
      userId: id, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    console.error(`[AdminUsers:ToggleStatus:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
    ...
    return res.status(500).json({
      success: false,
      message: 'User status toggle failed'
    });
  }
});
...
router.put('/:id', requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    console.info('[AdminUsers:Update]', { 
      requestId,
      userId: id, 
      requester: req.user?.email || req.session?.user?.email
    });
    ...
    // Fetch updated user data with fixed SQL
    ...
    console.error(`[AdminUsers:Update:${requestId}] Error:`, {
      code: error.code,
      errno: error.errno,
      sqlMessage: error.sqlMessage,
      message: error.message,
