    ...
    (async () => {
      try {
        ...
        const [churches] = await promisePool.query(
          'SELECT * FROM orthodmetrics_dev.churches WHERE id = ?',
          [churchId]
        );
...
router.post('/', upload.single('logo'), async (req, res) => {
  try {
    ...
    const [existingChurches] = await promisePool.query(
      'SELECT id FROM orthodmetrics_dev.churches WHERE name = ? AND is_active = 1',
      [name]
    );
    ...
    const [existingEmails] = await promisePool.query(
      'SELECT id FROM orthodmetrics_dev.churches WHERE admin_email = ? AND is_active = 1',
      [admin_email]
    );
...
router.get('/', async (req, res) => {
  try {
    const [churches] = await promisePool.execute(`
      ...
      FROM orthodmetrics_dev.churches 
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [rows] = await promisePool.query(
      `SELECT * FROM orthodmetrics_dev.churches WHERE id = ?`, [churchId]
    );
    if (rows.length === 0) {
  ...
  try {
    ...
    if (updateData.church_id && updateData.church_id !== currentChurchId) {
      ...
      const [existingChurch] = await promisePool.execute(
        'SELECT id FROM orthodmetrics_dev.churches WHERE id = ?',
        [newChurchId]
      );
    ...
    if (isChurchIdUpdate) {
      ...
      const insertQuery = `
        INSERT INTO orthodmetrics_dev.churches 
      ...
      await promisePool.execute(
        'DELETE FROM orthodmetrics_dev.churches WHERE id = ?',
        [currentChurchId]
      );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const [result] = await promisePool.execute(
      'UPDATE orthodmetrics_dev.churches SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [churchId]
    );
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    const [result] = await promisePool.execute(
      'UPDATE orthodmetrics_dev.churches SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [active ? 1 : 0, churchId]
    );
