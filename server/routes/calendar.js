...
router.put('/tasks/:id', requireAuth, async (req, res) => {
  try {
    ...
    const [existingTasks] = await promisePool.execute(
    ...
    await promisePool.execute(query, updateValues);
...
router.get('/tasks/stats', requireAuth, async (req, res) => {
  try {
    ...
    const [agentStats] = await promisePool.execute(`
      SELECT agent, COUNT(*) as count
      FROM ai_tasks
      GROUP BY agent
    `);

    const [priorityStats] = await promisePool.execute(`
