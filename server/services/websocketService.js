...
class WebSocketService {
    ...
    /**
     * Send admin message to a specific user (instant notification)
     */
    async sendAdminMessage(userId, message, senderId = null) {
        try {
            const notification = {
                type: 'admin_message',
                title: 'Admin Message',
            ...
            if (this.userSockets.has(userId)) {
                this.io.to(`user_${userId}`).emit('admin_message', notification);
                console.log(`📨 Admin message sent to user ${userId} (online)`);
            } else {
                console.log(`📨 Admin message queued for user ${userId} (offline)`);
            }

            // Always save to database
            // Get notification_type_id for 'admin_message' (or use 92 if it exists)
            const [typeResult] = await promisePool.query(`
                SELECT id FROM notification_types WHERE name = 'admin_message' LIMIT 1
            `);
            
            let notificationTypeId = 92; // Default to the ID we just created
            if (typeResult.length > 0) {
                notificationTypeId = typeResult[0].id;
            }

            const [insertResult] = await promisePool.query(`
                INSERT INTO notifications (user_id, notification_type_id, title, message, sender_id, data, priority, is_read)
                VALUES (?, ?, 'Admin Message', ?, ?, ?, 'high', 0)
            `, [
                userId,
                notificationTypeId,
                message,
            ...
            // Get the full notification with type name and category
            const [fullNotification] = await promisePool.query(`
                SELECT 
                    n.*,
            ...
            // Also emit as new_notification so it appears in the notifications icon
            if (this.userSockets.has(userId) && fullNotification.length > 0) {
                ...
                console.log(`📬 Notification ${notificationId} sent to user ${userId} via WebSocket`);
            }

            return { success: true, delivered: this.userSockets.has(userId) };
