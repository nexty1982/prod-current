...
const hasOMSession = req.headers.cookie?.includes('orthodoxmetrics.sid=');
  ...
  // ========================================================================
  // Environment Gating: Inject X-Environment header based on user role
  // ========================================================================
  // super_admin users get access to "latest" environment (bleeding-edge features)
  // All other roles get "stable" environment (production-ready features)
  const userEnvironment = req.user.role === 'super_admin' ? 'latest' : 'stable';
  res.setHeader('X-Environment', userEnvironment);
  res.setHeader('X-User-Role', req.user.role);
  
  // Also add to request object for use by other middleware/routes
  req.environment = userEnvironment;
  
  // Log successful authentication
  console.log('✅ Authentication successful for:', req.user.email, 'Role:', req.user.role, 'Environment:', userEnvironment);
