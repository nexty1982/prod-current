...
exports.transformMadgeToOM = transformMadgeToOM;
const fs = require("fs");
...
function getUniqueFileName(filePath, fileNameMap) {
...
function shouldSkipConnection(fromFile, toFile) {
...
function getConnectionPriority(fromFile, toFile, fromType, toType) {
    let priority = 0;
...
function capNodeConnections(nodes, connections, maxConnections = 8) {
    const skippedEdges = [];
    ...
    const prioritizedConnections = connections.map(conn => {
        ...
        if (!fromNode || !toNode)
            return { ...conn, priority: -1 };
        const priority = getConnectionPriority(fromNode.path || '', toNode.path || '', fromNode.type, toNode.type);
...
async function transformMadgeToOM() {
    ...
    const fileNameMap = new Map(); // for deduplication
    ...
    const potentialFrontendFiles = dependencyNodes.filter(n => n.path && (n.path.includes('front-end/src/') ||
        n.path.endsWith('.tsx') ||
        n.path.endsWith('.jsx') ||
        n.path.endsWith('.vue') ||
        n.path.endsWith('.css') ||
        n.path.endsWith('.scss'))).slice(0, 5);
    ...
    const potentialDbFiles = dependencyNodes.filter(n => n.path && (n.path.includes('/db/') ||
        n.path.includes('/models/') ||
        n.path.includes('/database/') ||
        n.path.endsWith('.sql') ||
        n.path.endsWith('.model.js') ||
        n.path.endsWith('.model.ts'))).slice(0, 5);
