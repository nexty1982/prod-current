...
import { Col, Offcanvas, OffcanvasBody, OffcanvasHeader, Row } from 'react-bootstrap'

import SimplebarReactClient from './wrappers/SimplebarReactClient'
import type { DialogControlType, MenuType, ThemeType } from '@/types/context'
...
const ThemeCustomizer = ({ open, toggle }: DialogControlType) => {
  const { resetSettings, theme } = useLayoutContext()

  return (
    <div>
      <Offcanvas show={open} onHide={toggle} placement="end" className="border-0" tabIndex={-1}>
        <OffcanvasHeader closeButton closeVariant="white" className="d-flex align-items-center bg-primary p-3">
        ...
        <div className="offcanvas-footer border-top p-3 text-center">
          <Row>
            <Col>
              <button type="button" onClick={resetSettings} className="btn btn-danger w-100">
                Reset
              </button>
            </Col>
          </Row>
