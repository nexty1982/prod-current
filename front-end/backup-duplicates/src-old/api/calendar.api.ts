 ...
 * Handles AI task management, Kanban synchronization, liturgical calendar data, and real-time updates
 */
...
class CalendarAPI {
  ...
  liturgical = {
    // Get liturgical info for a specific date
    getDay: async (date: string, params?: {
      calendarSystem?: 'gregorian' | 'julian' | 'revised-julian';
      timezone?: string;
      language?: string;
    }): Promise<LiturgicalDay> => {
      const queryParams = new URLSearchParams(params as any).toString();
      return apiClient.get(`/api/calendar/${date}?${queryParams}`);
    },

    // Get liturgical info for a month
    getMonth: async (year: number, month: number, params?: {
      calendarSystem?: 'gregorian' | 'julian' | 'revised-julian';
      timezone?: string;
      language?: string;
    }): Promise<CalendarMonth> => {
      const queryParams = new URLSearchParams(params as any).toString();
      return apiClient.get(`/api/calendar/month/${year}/${month}?${queryParams}`);
    },

    // Get major feasts list
    getFeasts: async (params?: {
      year?: number;
      type?: string;
      rank?: string;
      language?: string;
      ...
      const queryParams = new URLSearchParams(params as any).toString();
      return apiClient.get(`/api/calendar/feasts?${queryParams}`);
    },

    // Get next upcoming feast
    getNextFeast: async (from?: string): Promise<Feast> => {
      const queryParams = from ? `?from=${from}` : '';
      return apiClient.get(`/api/calendar/next-feast${queryParams}`);
    },

    // Search saints by name or type
    searchSaints: async (query: string, params?: {
      type?: string;
      limit?: number;
      language?: string;
    }): Promise<Saint[]> => {
      ...
      return apiClient.get(`/api/calendar/saints/search?${queryParams}`);
    },

    // Get saint details
    getSaint: async (id: string, language?: string): Promise<Saint> => {
      const queryParams = language ? `?language=${language}` : '';
      return apiClient.get(`/api/calendar/saints/${id}${queryParams}`);
    },

    // Search calendar (feasts, saints, readings)
    search: async (query: string, params?: {
      type?: 'all' | 'feasts' | 'saints' | 'readings';
      dateRange?: { start: string; end: string };
      language?: string;
      limit?: number;
    }): Promise<{
      ...
      return apiClient.get(`/api/calendar/search?${queryParams}`);
    },

    // Get readings for date or feast
    getReadings: async (date: string, params?: {
      feastId?: string;
      type?: 'all' | 'epistle' | 'gospel' | 'matins' | 'vespers';
      language?: string;
    }) => {
      const queryParams = new URLSearchParams(params as any).toString();
      return apiClient.get(`/api/calendar/readings/${date}?${queryParams}`);
    },

    // Export calendar data
    export: async (options: ShareOptions & {
      dateRange: { start: string; end: string };
    }) => {
      return apiClient.post('/api/calendar/export', options, {
        responseType: options.format === 'pdf' || options.format === 'ical' ? 'blob' : 'json'
      });
    },

    // Share specific day
    shareDay: async (date: string, options: ShareOptions) => {
      return apiClient.post(`/api/calendar/share/${date}`, options);
    },

    // Get calendar with filters
    getFiltered: async (filters: CalendarFilters) => {
      return apiClient.post('/api/calendar/filter', filters);
    }
  };
}
