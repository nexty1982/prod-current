/**
 * HomepageRecordsTransformSection — Interactive records transformation showcase.
 * Replaces the static "Record Types" section on the homepage.
 * Adapted from Figma TransformationShowcase design.
 */
...
import { RecordTypeCards } from './RecordTypeCards';
import { EnhancedRecordViewer } from './EnhancedRecordViewer';
import { RecordsDataTable, type Column } from './RecordsDataTable';
...
export default function HomepageRecordsTransformSection() {
  const { t } = useLanguage();
  const [activeType, setActiveType] = useState<RecordType>('baptisms');
    ...
    <section className="py-16 md:py-20 bg-[#2d1b4e] dark:bg-gray-950 text-white">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        ...
        <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl shadow-purple-900/30 bg-black/20 backdrop-blur" style={{ aspectRatio: '16/10' }}>
          ...
          >
            {state === 'digital' && <DigitalContent type={activeType} />}
          </motion.div>
