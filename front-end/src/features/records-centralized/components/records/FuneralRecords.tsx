/**
 * Funeral Records Implementation
...
import { UnifiedRecordForm } from '@/features/records-centralized/components/forms/UnifiedRecordForm';
import { RECORD_TYPES, FIELD_DEFINITIONS, THEME_COLORS } from '@/constants';

// Types
interface FuneralRecordsProps {
  churchId: string;
  PDFDocument?: any;
  ReadOnlyView?: React.ComponentType;
}

export function FuneralRecords({
  churchId,
  PDFDocument,
  ReadOnlyView,
}: FuneralRecordsProps) {
  const recordType = RECORD_TYPES.FUNERAL;
  ...
  const columns = useMemo(() => {
    return fieldDefs.tableColumns.map(column => ({
      ...
      render: (value: any, record: any) => {
        ...
        if (column.cellRenderer === 'numericColumn') {
  ...
  const searchFilters = useMemo(() => [
    ...
    },
    {
      ...
      options: dropdownOptions.locations?.map((loc: any) => ({
        value: loc.id,
        label: loc.name
      })) || [],
    },
  ...
  const handleDeleteRecord = async (record: any) => {
    if (window.confirm(`Are you sure you want to delete the funeral record for ${fieldDefs.displayName(record)}?`)) {
      try {
  ...
  const handleExport = async (format: 'csv' | 'pdf' | 'excel') => {
    try {
      ...
      if (result.success && result.data) {
        ...
        link.href = url;
        link.download = `funeral-records-export.${format}`;
        link.click();
  ...
  const handleGenerateCertificate = (record: any) => {
    ...
    console.log('Generating funeral certificate for:', record);
  ...
  if (isLoading) {
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <Typography>Loading funeral records...</Typography>
      </Box>
    );
  }
  ...
  if (error) {
    return (
      <Alert severity="error" sx={{ m: 2 }}>
        Error loading funeral records: {error}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <FlowerIcon sx={{ color: 'white', fontSize: 32 }} />
        ...
        <Box sx={{ p: 3, pb: 0 }}>
          <RecordsSearch
            ...
            searchPlaceholder="Search funeral records..."
          />
        </Box>
        ...
        <Box sx={{ p: 3, pt: 0 }}>
          <RecordsTable
            ...
            emptyMessage="No funeral records found"
          />
        </Box>
      ...
      <RecordsModal
        ...
        title="Import Funeral Records"
        maxWidth="sm"
      >
        <Box sx={{ p: 2 }}>
          <Typography variant="body1" sx={{ mb: 2 }}>
            Select a file to import funeral records. Supported formats: CSV, Excel.
          </Typography>
          <input
            type="file"
            accept=".csv,.xlsx,.xls"
