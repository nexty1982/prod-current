...
interface UnifiedRecordsPageProps {
  defaultRecordType?: string;
}

const UnifiedRecordsPage: React.FC<UnifiedRecordsPageProps> = ({ defaultRecordType = 'baptism' }) => {
  ...
  const [selectedRecordType, setSelectedRecordType] = useState<string>(() => {
    // Initialize from URL parameter first, then fall back to prop
    const typeFromUrl = searchParams.get('type');
    return typeFromUrl || defaultRecordType;
  });
  ...
  const fetchRecords = async (
    ...
    try {
      ...
      const apiService = createRecordsApiService(
        (churchId && churchId !== 0) ? churchId.toString() : '0'
      );
      
      const response = await apiService.listRecords(selectedType.apiEndpoint, {
        page: requestPage,
        limit: requestLimit,
      ...
      if (response.success && response.data) {
        ...
        const total = response.data.totalRecords ?? response.data.pagination?.total ?? response.data.records.length;
        ...
        throw new Error(response.error || 'Failed to fetch records from API');
      }
  ...
  // When searching: preserve server relevance ordering (don't re-sort client-side)
  // When not searching: apply local column sort
  const filteredAndSortedRecords = useMemo(() => {
    let result = [...records];

    // Only apply client-side sort when NOT searching (server handles relevance ordering)
    if (!debouncedSearch) {
      result.sort((recordA, recordB) => {
    ...
    return result;
  }, [records, sortConfig, debouncedSearch, showBestMatches]);
  ...
  const agGridComponents = useMemo(() => ({
    agGridStatusRenderer: (params: ICellRendererParams) => {
                    ...
                    <TableHead>
                      ...
                      >
                        {getColumnDefinitions(selectedRecordType).map((column: any, index: number) => (
                          <TableCell key={index} sx={{ ...getTableCellStyle('header'), fontWeight: 'bold', '& .MuiTableSortLabel-root': { color: 'inherit' }, '& .MuiTableSortLabel-root.Mui-active': { color: 'inherit' }, '& .MuiTableSortLabel-icon': { color: 'inherit !important' } }}>
                            <TableSortLabel
                        ...
                        <TableCell sx={{ 
                          ...
                          position: 'sticky',
                          right: 0,
                          backgroundColor: getTableHeaderStyle().backgroundColor || (isDarkMode ? 'primary.dark' : 'primary.main'),
                          zIndex: 2,
                        }} align="center">
                          Actions
                        </TableCell>
                        ...
                        paginatedRecords.map((record, index) => (
                          <TableRow
                            ...
                            sx={{
                              ...
                              ...(isDarkMode ? { backgroundColor: index % 2 === 0 ? 'background.default' : 'background.paper' } : {}),
                              border: selectedElement === 'row' ? '2px solid' : 'none',
                              borderColor: selectedElement === 'row' ? 'primary.main' : 'transparent',
                              cursor: 'pointer',
                              '&:hover': {
                          ...
                          >
                            {getColumnDefinitions(selectedRecordType).map((column: any, colIndex: number) => {
                              ...
                              return (
                                <TableCell key={colIndex} sx={{ ...getTableCellStyle('body'), ...(isDarkMode ? { backgroundColor: 'transparent', color: 'text.primary', borderColor: 'divider' } : {}) }}>
                            ...
                            <TableCell sx={{ 
                              ...
                              backgroundColor: isDarkMode
                                ...
                                : (getTableRowStyle(index % 2 === 0 ? 'even' : 'odd').backgroundColor || (index % 2 === 0 ? '#f5f5f5' : '#fff')),
                              ...(isDarkMode ? { color: 'text.primary', borderColor: 'divider' } : {}),
                              zIndex: 1,
                            }} align="center">
