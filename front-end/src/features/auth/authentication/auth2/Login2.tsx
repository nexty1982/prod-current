...
@keyframes float1 {
  0%, 100% { transform: translateY(0px) scale(1); opacity: var(--start-opacity); }
  50% { transform: translateY(-12px) scale(1.15); opacity: calc(var(--start-opacity) + 0.15); }
}
@keyframes float2 {
  0%, 100% { transform: translateY(0px) translateX(0px); opacity: var(--start-opacity); }
  50% { transform: translateY(-8px) translateX(6px); opacity: calc(var(--start-opacity) + 0.1); }
}
@keyframes float3 {
  ...
  return (
    <PageContainer title="Login" description="this is Login page">
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        >
          {/* Decorative gold orbs — animated */}
          <Box sx={{ position: 'absolute', top: 30, left: 40, width: 18, height: 18, borderRadius: '50%', background: 'radial-gradient(circle, #D4AF37 0%, rgba(212,175,55,0.3) 100%)', '--start-opacity': isDark ? 0.8 : 0.5, animation: 'float1 4s ease-in-out infinite' } as any} />
          <Box sx={{ position: 'absolute', top: 80, left: 100, width: 12, height: 12, borderRadius: '50%', background: 'radial-gradient(circle, #D4AF37 0%, rgba(212,175,55,0.3) 100%)', '--start-opacity': isDark ? 0.5 : 0.3, animation: 'float2 5s ease-in-out infinite 0.5s' } as any} />
          <Box sx={{ position: 'absolute', top: 60, right: 80, width: 80, height: 80, borderRadius: '50%', border: `2px solid ${isDark ? 'rgba(138,43,226,0.25)' : 'rgba(123,79,158,0.15)'}`, opacity: 0.4, animation: 'float3 6s ease-in-out infinite 1s' }} />
          <Box sx={{ position: 'absolute', bottom: 40, right: 60, width: 14, height: 14, borderRadius: '50%', background: 'radial-gradient(circle, #9B59B6 0%, rgba(155,89,182,0.2) 100%)', '--start-opacity': isDark ? 0.6 : 0.35, animation: 'float1 5.5s ease-in-out infinite 0.8s' } as any} />
          <Box sx={{ position: 'absolute', bottom: 100, left: 200, width: 10, height: 10, borderRadius: '50%', background: 'radial-gradient(circle, #D4AF37 0%, rgba(212,175,55,0.3) 100%)', '--start-opacity': isDark ? 0.4 : 0.25, animation: 'float2 4.5s ease-in-out infinite 1.2s' } as any} />
          ...
          <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
            <Grid container spacing={6} alignItems="center">
              ...
              <Grid item xs={12} md={6}>
                ...
                {/* Feature bullets */}
                <Stack spacing={1.5} sx={{ display: { xs: 'none', md: 'flex' }, animation: 'fadeSlideUp 0.8s ease-out 0.45s both' }}>
                  {features.map((f, i) => (
