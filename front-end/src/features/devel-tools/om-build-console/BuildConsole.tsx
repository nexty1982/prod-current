...
const BuildConsole: React.FC = () => {
  ...
  const [frontendOutput, setFrontendOutput] = useState<string>('');
  const [serverOutput, setServerOutput] = useState<string>('');
  const [frontendStatus, setFrontendStatus] = useState<'idle' | 'running' | 'success' | 'error'>('idle');
  ...
  const startBuildStreaming = () => {
    ...
    const connectionTimeout = setTimeout(() => {
      if (buildStatus === 'running' && !buildOutput && !frontendOutput && !serverOutput) {
        ...
        switch (data.type) {
          case 'start':
            if (config.buildTarget === 'dual') {
              // Dual build - handle separate streams
              if (data.target === 'frontend') {
                setFrontendOutput(prev => prev + data.message + '\n');
              } else if (data.target === 'server') {
                setServerOutput(prev => prev + data.message + '\n');
              } else {
          ...
          case 'output':
            if (config.buildTarget === 'dual') {
              // Dual build - route to appropriate output
              if (data.target === 'frontend') {
                setFrontendOutput(prev => prev + data.data);
              } else if (data.target === 'server') {
                setServerOutput(prev => prev + data.data);
              } else {
          ...
          case 'error':
            if (config.buildTarget === 'dual') {
              if (data.target === 'frontend') {
                setFrontendOutput(prev => prev + '[ERROR] ' + data.data);
                setFrontendStatus('error');
              } else if (data.target === 'server') {
                setServerOutput(prev => prev + '[ERROR] ' + data.data);
          ...
          case 'status':
            if (config.buildTarget === 'dual' && data.target) {
              if (data.target === 'frontend') {
                setFrontendStatus(data.status);
              } else if (data.target === 'server') {
          ...
          case 'complete':
            if (config.buildTarget === 'dual') {
              // Handle dual build completion
              if (data.target === 'frontend') {
                setFrontendStatus(data.success ? 'success' : 'error');
              } else if (data.target === 'server') {
                setServerStatus(data.success ? 'success' : 'error');
              } else {
                // Overall completion - both builds are done
                setBuildStatus(data.success ? 'success' : 'error');
                if (!data.success) {
                  setError('One or more builds failed');
                }
                setIsRunning(false);
                eventSource.close();
                loadBuildHistory();
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} sm={6} md={3}>
                  <FormControl fullWidth size="small" disabled={isRunning}>
                    ...
                    <Select
                      value={config.buildTarget || 'frontend'}
                      label="Build Target"
                      onChange={(e) => setConfig({ ...config, buildTarget: e.target.value as 'frontend' | 'server' | 'dual' })}
                    >
                      <MenuItem value="frontend">Frontend (front-end/package.json)</MenuItem>
                      <MenuItem value="server">Server (server/package.json)</MenuItem>
                      <MenuItem value="dual">Dual (Frontend + Server simultaneously)</MenuItem>
                    </Select>
                  </FormControl>
                </Grid>
          ...
          <BlankCard sx={{ height: '100%' }}>
            <CardContent sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              ...
              <Box sx={{ mb: 3, flex: 1 }}>
                ...
                {buildMeta && (
                  <Paper variant="outlined" sx={{ p: 2, mb: 2, bgcolor: isDark ? 'grey.800' : 'grey.50' }}>
          ...
          <BlankCard sx={{ 
            ...
            <CardContent sx={{ p: 0 }}>
              ...
              <Paper 
                ...
                sx={{ 
                  p: 2, 
                  bgcolor: isDark ? 'grey.900' : 'grey.800',
                  color: 'white',
                  borderRadius: 0
                }}
              >
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Box sx={{ 
                      ...
                      height: 12, 
                      borderRadius: '50%', 
                      bgcolor: buildStatus === 'success' ? theme.palette.success.main : 
                               buildStatus === 'error' ? theme.palette.error.main : 
                               buildStatus === 'running' ? theme.palette.info.main : theme.palette.grey[500],
                      mr: 2,
              ...
              ) : config.buildTarget === 'dual' ? (
                /* Dual Build Split Output Display */
                <Grid container spacing={2} sx={{ minHeight: 350, maxHeight: 600 }}>
                  <Grid item xs={12} md={6}>
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#c9d1d9' }}>
                        🎨 Frontend Build
                      </Typography>
                      <Chip 
                        label={frontendStatus === 'running' ? 'Running' : frontendStatus === 'success' ? 'Success' : frontendStatus === 'error' ? 'Error' : 'Idle'}
                        size="small"
                        color={frontendStatus === 'success' ? 'success' : frontendStatus === 'error' ? 'error' : frontendStatus === 'running' ? 'primary' : 'default'}
                        sx={{ height: '20px' }}
                      />
                    </Box>
                    <Box
                      sx={{
                        ...
                        lineHeight: 1.6,
                        whiteSpace: 'pre-wrap',
                        border: `1px solid ${isDark ? '#30363d' : '#404040'}`,
                        borderRadius: 1,
                        scrollbarWidth: 'thin',
                        '&::-webkit-scrollbar': {
                          width: '8px',
                        ...
                        '&::-webkit-scrollbar-track': {
                          background: isDark ? '#21262d' : '#2d2d2d',
                        },
                        '&::-webkit-scrollbar-thumb': {
                          background: isDark ? '#30363d' : '#404040',
                          borderRadius: '4px',
                        },
                        '&::-webkit-scrollbar-thumb:hover': {
                          background: isDark ? '#484f58' : '#505050',
                        }
                      }}
                    >
                      {frontendOutput ? (
                        <Box>
                          {frontendOutput.split('\n').map((line, index) => (
                            <Box 
                              key={index} 
                              sx={{ 
                                display: 'flex',
                            ...
                            >
                              ...
                              <Box sx={{ flex: 1, wordBreak: 'break-word', color: '#c9d1d9' }}>
                                {line || ' '}
                              </Box>
                            </Box>
                          ))}
                        ...
                        <Box sx={{ 
                          ...
                          <Typography variant="body2" sx={{ color: '#7d8590' }}>
                            Waiting for frontend build output...
                          </Typography>
                        </Box>
                      )}
                    </Box>
                  ...
                  <Grid item xs={12} md={6}>
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <Chip 
                        label={serverStatus === 'running' ? 'Running' : serverStatus === 'success' ? 'Success' : serverStatus === 'error' ? 'Error' : 'Idle'}
                        size="small"
                        color={serverStatus === 'success' ? 'success' : serverStatus === 'error' ? 'error' : serverStatus === 'running' ? 'primary' : 'default'}
                        sx={{ height: '20px' }}
                      />
                    </Box>
                    <Box
                    ...
                    >
                      {serverOutput ? (
                        ...
                        <Box sx={{ 
                          ...
                          <Typography variant="body2" sx={{ color: '#7d8590' }}>
                            Waiting for server build output...
                          </Typography>
                        </Box>
                      )}
                    </Box>
