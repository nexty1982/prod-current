...
const STATUS_CONFIG: Record<StatusType, { label: string; bg: string; text: string; dot: string }> = {
  ...
  review:     { label: "Needs Review", bg: "bg-amber-50", text: "text-amber-700", dot: "bg-amber-500" },
  approved:   { label: "Approved",   bg: "bg-green-50",  text: "text-green-700", dot: "bg-green-500" },
  draft:      { label: "Draft",      bg: "bg-slate-100", text: "text-slate-600", dot: "bg-slate-400" },
  uploading:  { label: "Uploading",  bg: "bg-blue-50",   text: "text-blue-700",  dot: "bg-blue-500" },
  uploaded:   { label: "Uploaded",   bg: "bg-green-50",  text: "text-green-700", dot: "bg-green-500" },
  warning:    { label: "Warning",    bg: "bg-amber-50",  text: "text-amber-700", dot: "bg-amber-500" },
  seeded:     { label: "Seeded",     bg: "bg-purple-50", text: "text-purple-700", dot: "bg-purple-500" },
  needs_correction: { label: "Needs Correction", bg: "bg-amber-50", text: "text-amber-700", dot: "bg-amber-500" },
...
export function RecordTypeBadge({ type }: { type: "baptism" | "marriage" | "funeral" | "unknown" }) {
  const config = {
    baptism: { label: "Baptism", bg: "bg-blue-50", text: "text-blue-700" },
    marriage: { label: "Marriage", bg: "bg-purple-50", text: "text-purple-700" },
    funeral: { label: "Funeral", bg: "bg-slate-100", text: "text-slate-600" },
    unknown: { label: "Auto-detect", bg: "bg-amber-50", text: "text-amber-700" },
  }[type];
  return (
...
export function ConfidenceBadge({ value }: { value: number }) {
  const color = value >= 85 ? "text-green-700 bg-green-50" : value >= 65 ? "text-amber-700 bg-amber-50" : "text-red-700 bg-red-50";
  return (
