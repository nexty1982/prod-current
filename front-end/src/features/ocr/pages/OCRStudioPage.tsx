  ...
  const routes = useMemo(() => {
    if (isPortal) {
      return {
        ...
        review: effectiveChurchId ? `/portal/ocr/review/${effectiveChurchId}` : '/portal/ocr/upload',
        records: '/portal/records-management',
        layoutTemplates: '/devel/ocr-studio/layout-templates',
      };
    }
    ...
    return {
      ...
      records: '/portal/records-management',
      layoutTemplates: `/devel/ocr-studio/layout-templates${churchQuery}`,
    };
  }, [isPortal, effectiveChurchId]);

  // Fix review route with church query param
  const reviewRoute = useMemo(() => {
    if (isPortal) {
      return effectiveChurchId ? `/portal/ocr/review/${effectiveChurchId}` : '/portal/ocr/upload';
    }
    if (!effectiveChurchId) return '/devel/ocr-studio/upload';
    return `/devel/ocr-studio/review/${effectiveChurchId}?church=${effectiveChurchId}`;
  }, [isPortal, effectiveChurchId]);

  const content = (
    <>
      {!inShell && <OcrStudioNav />}
      {!inShell && <OcrChurchSelector />}
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ height: '100%' }}>
            <CardContent>
              ...
              <Button variant="outlined" fullWidth onClick={() => navigate(reviewRoute)}>Open Review</Button>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ height: '100%' }}>
            <CardContent>
              ...
              <Typography variant="h6" gutterBottom>Layout Templates</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Era-specific ledger extraction templates
              </Typography>
              <Button variant="outlined" fullWidth onClick={() => navigate(routes.layoutTemplates)}>Open Templates</Button>
            </CardContent>
          </Card>
        </Grid>
      </Grid>
