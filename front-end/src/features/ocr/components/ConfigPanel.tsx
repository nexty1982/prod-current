...
const ConfigPanel: React.FC<ConfigPanelProps> = ({ trigger, churchId, onSettingsChange }) => {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      console.log('[ConfigPanel] Saving settings:', { churchId, settings });
      await updateSettings(settings, churchId);
      console.log('[ConfigPanel] Settings saved successfully');
      onSettingsChange?.(settings);
      // Reload settings to verify they were saved
      await loadSettings();
      setOpen(false);
    } catch (error: any) {
      console.error('[ConfigPanel] Failed to save OCR settings:', error);
