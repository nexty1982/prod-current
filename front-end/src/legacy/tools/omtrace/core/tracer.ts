...
export async function traceDependencies(
  ...
  const transitiveImports = options.deep ? findTransitiveImports(targetPath, index, new Set()) : [];
...
function findTransitiveImports(
  ...
  if (depth > 10) {
    log.warn('Max depth reached for transitive tracing', { targetPath, depth });
    return [];
  }
  ...
  for (const importPath of targetNode.imports) {
    if (!visited.has(importPath)) {
      ...
      const nestedImports = findTransitiveImports(importPath, index, visited, depth + 1);
...
async function findServerEndpoints(targetPath: string, index: DepsIndex): Promise<string[]> {
  ...
  try {
    ...
    const path = await import('path');
       ...
       let fullPath: string;
       if (targetPath.startsWith('src/') || targetPath.includes('/admin/') || targetPath.includes('/shared/ui/legacy/') || targetPath.includes('/api/') || targetPath.includes('/context/')) {
         // This is a front-end file, look for API calls
         // Reconstruct the full path by adding the missing 'src/' prefix
         const normalizedPath = targetPath.startsWith('src/') ? targetPath : 'src/' + targetPath;
         fullPath = path.default.join('/var/www/orthodoxmetrics/prod/front-end', normalizedPath);
       } else {
  ...
  for (const node of index.nodes) {
    if (node.id.startsWith('server/src/')) {
      ...
      try {
        ...
        const fullPath = path.default.join('/var/www/orthodoxmetrics/prod', node.id);
