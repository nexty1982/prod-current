    ...
    const startUpload = async (): Promise<void> => {
        try {
            ...
            for (const file of files) {
                ...
                formData.append('church_id', user?.church_id?.toString() || '1'); // Use user's church or default
                formData.append('language', 'en'); // Default language
                formData.append('record_type', 'baptism'); // Default record type
                formData.append('submitted_by', user?.id?.toString() || '1'); // Use authenticated user ID
