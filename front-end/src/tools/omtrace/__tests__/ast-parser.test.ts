...
describe('ASTParser', () => {
  ...
  describe('Dynamic Parameter Extraction', () => {
    it('should extract dynamic parameters from route patterns', () => {
      const params = (astParser as any).extractDynamicParams('/apps/records/:churchId/member/:memberId');
  ...
  describe('Component Path Resolution', () => {
    it('should resolve relative imports', async () => {
      const mockExistsSync = vi.mocked(fs.existsSync);
      mockExistsSync.mockImplementation((filePath) => {
        return filePath === '/mock/front-end/src/components/Button.tsx';
      });
      ...
      const result = await astParser.resolveComponentPath('../components/Button', 'Button', mockSourceFile as any);
      expect(result).toBe('/mock/front-end/src/components/Button.tsx');
    });
    ...
    it('should resolve absolute imports with @ alias', async () => {
      ...
      const result = await astParser.resolveComponentPath('@/components/Button', 'Button');
      expect(result).toBe('/mock/front-end/src/components/Button.tsx');
    });
    ...
    it('should find index files when directory exists', async () => {
      const mockExistsSync = vi.mocked(fs.existsSync);
      mockExistsSync.mockImplementation((filePath) => {
        if (filePath === '/mock/front-end/src/components/Button') return false;
        if (filePath === '/mock/front-end/src/components/Button.tsx') return false;
        if (filePath === '/mock/front-end/src/components/Button/index.tsx') return true;
        return false;
      });

      const result = await astParser.resolveComponentPath('@/components/Button', 'Button');
      expect(result).toBe('/mock/front-end/src/components/Button/index.tsx');
    });
