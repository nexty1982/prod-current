...
## 2. Authenticate (omsvc service account)
...
1. **Review Ready** → Admin reviews the PR on GitHub
2. **Approved** → Admin approves the PR
3. **Done** → Admin merges the PR, complete-work is called
...
```
backlog → in_progress → self_review → review_ready → approved → done
                                          ↑ agent stops here
```
...
- Agents own: `in_progress`, `self_review`, `testing`
- Admins own: `backlog`, `triaged`, `planned`, `scheduled`, `review_ready`, `approved`, `done`
