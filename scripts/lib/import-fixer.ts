...
import { ImportError, FixCandidate, ImportFix, FixerOptions } from '../types.js';

export class ImportFixer {
  private fsIndex: FileSystemIndex;
  private rewriter: ImportRewriter;
  private errors: ImportError[] = [];
  private fixes: ImportFix[] = [];

  constructor(private options: FixerOptions) {
    this.fsIndex = new FileSystemIndex(options.root);
    this.rewriter = new ImportRewriter(options.root);
  }

  async run(): Promise<{ fixes: ImportFix[]; errors: ImportError[] }> {
    // Build file index
  ...
  private async collectViteErrors(): Promise<void> {
  ...
  private parseViteOutput(output: string): void {
  ...
  private async parseErrorLog(logPath: string): Promise<void> {
  ...
  private async scanStatically(): Promise<void> {
  ...
  private canResolveImport(fromFile: string, specifier: string): boolean {
  ...
  private async fixImportError(error: ImportError): Promise<void> {
  ...
  private findCandidates(error: ImportError): FixCandidate[] {
  ...
  private tryExtensionResolution(error: ImportError, candidates: FixCandidate[]): void {
    const extensions = ['.tsx', '.ts', '.jsx', '.js'];
  ...
  private tryCaseVariations(error: ImportError, basename: string, candidates: FixCandidate[]): void {
    const variations = [
      basename,
  ...
  private tryFolderFlips(error: ImportError, candidates: FixCandidate[]): void {
  ...
  private tryBasenameSearch(error: ImportError, basename: string, candidates: FixCandidate[]): void {
  ...
  private calculateProximity(from: string, to: string): number {
  ...
  private buildImportPath(fromFile: string, toFile: string): string {
  ...
  private createStub(error: ImportError): void {
    const specifier = error.specifier;
  ...
  private resolveStubPath(fromFile: string, specifier: string): string | null {
    if (specifier.startsWith('.')) {
      const fromDir = path.dirname(fromFile);
