...
export async function fetchTables(churchId: number): Promise<string[]> {
  try {
    const response = await fetch(`/api/churches/${churchId}/../features/records/records/tables`, {
      credentials: 'include'
    });
...
export async function fetchRecords(
  ...
  try {
    ...
    const response = await fetch(`/api/churches/${churchId}/records?${params}`, {
      credentials: 'include'
    });
...
export async function fetchRecordsEnhanced(
  ...
  try {
    ...
    const response = await fetch(`/api/../features/records/records/enhanced?${params}`, {
      credentials: 'include'
    });
