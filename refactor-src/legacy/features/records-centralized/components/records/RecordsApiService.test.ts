...
import { RecordsApiService, createRecordsApiService } from '@/RecordsApiService';
...
describe('RecordsApiService', () => {
  ...
  describe('getRecords', () => {
    it('should fetch records with default parameters', async () => {
      ...
      expect(mockApiJson).toHaveBeenCalledWith(
        `/api/churches/${mockChurchId}/../features/records/records/baptisms?`
      );
    ...
    it('should fetch records with filters and pagination', async () => {
      ...
      expect(mockApiJson).toHaveBeenCalledWith(
        `/api/churches/${mockChurchId}/../features/records/records/baptisms?status=active&search=test&sortBy=name&sortOrder=asc&page=2&limit=5`
      );
      expect(result.success).toBe(true);
    });
  ...
  describe('updateRecord', () => {
    it('should update an existing record', async () => {
      ...
      expect(mockApiJson).toHaveBeenCalledWith(
        `/api/churches/${mockChurchId}/../features/records/records/baptisms/1`,
        {
          method: 'PUT',
          body: JSON.stringify(recordData)
        }
  ...
  describe('searchRecords', () => {
    it('should search records with term and filters', async () => {
      ...
      expect(mockApiJson).toHaveBeenCalledWith(
        `/api/churches/${mockChurchId}/../features/records/records/baptisms/search?q=test%20search&status=active`
      );
      expect(result.success).toBe(true);
  ...
  describe('getKnownFields', () => {
    it('should fetch known fields for record type', async () => {
      ...
      expect(mockApiJson).toHaveBeenCalledWith('/api/../features/records/records/baptisms/known-fields');
      expect(result.success).toBe(true);
