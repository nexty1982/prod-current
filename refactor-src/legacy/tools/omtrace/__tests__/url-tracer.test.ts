...
describe('URLTracer', () => {
  ...
  describe('Route Matching', () => {
    ...
    it('should match dynamic routes with parameters', () => {
      const routes = [
        { urlPattern: '/apps/../features/records/records/:churchId', componentName: 'RecordsUI' },
        { urlPattern: '/admin/church/:id', componentName: 'ChurchAdmin' },
      ];

      const match = (urlTracer as any).findRouteMatch('/apps/../features/records/records/46', routes);
      
      expect(match).toBeDefined();
      expect(match.pattern).toBe('/apps/../features/records/records/:churchId');
      expect(match.params).toEqual({ churchId: '46' });
    });

    it('should handle multiple dynamic parameters', () => {
      const routes = [
        { urlPattern: '/apps/../features/records/records/:churchId/member/:memberId', componentName: 'MemberDetail' },
      ];

      const match = (urlTracer as any).findRouteMatch('/apps/../features/records/records/46/member/123', routes);
    ...
    it('should choose most specific route when multiple match', () => {
      const routes = [
        ...
        { urlPattern: '/apps/../features/records/records/:churchId', componentName: 'ChurchRecords' },
      ];

      const match1 = (urlTracer as any).findRouteMatch('/apps/records', routes);
      expect(match1.pattern).toBe('/apps/records'); // Exact match wins

      const match2 = (urlTracer as any).findRouteMatch('/apps/../features/records/records/46', routes);
      expect(match2.pattern).toBe('/apps/../features/records/records/:churchId'); // More specific than /apps/:type
    });
  });
  ...
  describe('Dynamic Parameter Extraction', () => {
    it('should extract dynamic parameters correctly', () => {
      const params = (urlTracer as any).extractDynamicParams(
        '/apps/../features/records/records/46/member/123', 
        '/apps/../features/records/records/:churchId/member/:memberId'
      );
    ...
    it('should handle missing parameters gracefully', () => {
      const params = (urlTracer as any).extractDynamicParams(
        '/apps/records', 
        '/apps/../features/records/records/:churchId'
      );
  ...
  describe('Menu Path Matching', () => {
    it('should match menu paths to route patterns', () => {
      ...
      const matches2 = (urlTracer as any).menuPathMatches(
        '/apps/../features/records/records/46',
        '/apps/../features/records/records/:churchId'
      );
  ...
  describe('Dependency Classification', () => {
    ...
    it('should classify components correctly', () => {
      const kind1 = (urlTracer as any).classifyDependency('src/shared/ui/legacy/Button.tsx', 'Button');
    ...
    it('should classify styles correctly', () => {
      ...
      const kind2 = (urlTracer as any).classifyDependency('src/shared/ui/legacy/Button.scss', 'Button.scss');
