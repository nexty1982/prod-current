...
const RecordsAgGrid: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchTables = async () => {
            try {
                const response = await fetch(`/a../features/records/records/tables?db=${selectedChurch.database_name}`, {
                    credentials: 'include',
        ...
        const fetchRecordsData = async () => {
            ...
            try {
                ...
                const response = await fetch(
                    `/a../features/records/records/enhanced?db=${selectedChurch.database_name}&table=${selectedTable}&churchId=${churchId}`, 
                    {
                        credentials: 'include',
                        headers: { 'Accept': 'application/json' },
                    }
        ...
        const handleMessage = (event: MessageEvent) => {
            if (event.data?.type === 'FIELD_MAPPING_SAVED' && 
                ...
                if (selectedChurch?.database_name && selectedTable) {
                    const churchId = selectedChurch.id || selectedChurch.church_id;
                    fetch(`/a../features/records/records/enhanced?db=${selectedChurch.database_name}&table=${selectedTable}&churchId=${churchId}`, {
                        credentials: 'include',
                        headers: { 'Accept': 'application/json' },
                    })
